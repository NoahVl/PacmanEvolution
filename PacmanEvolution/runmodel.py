"""
Original run.py made to run a NEAT neural net model.
...
# Load best genome.
genomeFile = 'bestGenome/7_testTwoGhostsRandom_609.p'
genome = pickle.load(open(genomeFile, 'rb'))

fitnessScores = []

for i in range(10):
    # If we have to randomize the map, to prevent against overfitting.
    if layouts[0].find("Random"):
        # Randomize dot spots to train model
        randomize_layout(layouts[0])

    gstate, stats = run(layouts[0], pacman_agent, display, args.speed, args.ghosts, args.timeouts, genome, config)
    fitness = gstate.score
